...
export async function cropGif(
  ...
  if (mode === 'featured') {
    ...
    const result = await runCrop(
      buffer,
      `--optimize=3 --crop ${x},0+630x${height} input.gif -o /out/featured_main.gif`,
    );
  ...
  const [mainBuf, sideBuf] = await Promise.all([
    runCrop(buffer, `--optimize=3 --crop ${mainX},0+506x${height} input.gif -o /out/main.gif`),
    runCrop(buffer, `--optimize=3 --crop ${sideX},0+100x${height} input.gif -o /out/side.gif`),
  ]);
